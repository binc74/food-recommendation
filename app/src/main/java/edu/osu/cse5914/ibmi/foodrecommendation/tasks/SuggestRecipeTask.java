...
public class SuggestRecipeTask extends AsyncTask<String, Void, ArrayList> {
    ...
    protected ArrayList doInBackground(String... objects) {
        ...
        restaurantList.add( new Restaurant( name1, rcp1.replaceAll( "\\s+","%20" ), url1, displayedPhone ) );//use rating=1 and prepTime=2 for now and change later
        restaurantList.add( new Restaurant( name2, rcp2.replaceAll( "\\s+","%20" ), url2, displayedPhone ) );
        restaurantList.add( new Restaurant( name3, rcp3.replaceAll( "\\s+","%20" ), url3, displayedPhone ) );
        restaurantList.add( new Restaurant( name4, rcp4.replaceAll( "\\s+","%20" ), url4, displayedPhone ) );//use rating=1 and prepTime=2 for now and change later
        restaurantList.add( new Restaurant( name5, rcp5.replaceAll( "\\s+","%20" ), url5, displayedPhone ) );
        restaurantList.add( new Restaurant( name6, rcp6.replaceAll( "\\s+","%20" ), url6, displayedPhone ) );
